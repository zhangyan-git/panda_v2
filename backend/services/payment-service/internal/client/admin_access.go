...
// coffee-machine-service 里那几个同名类型是同一份实现，各服务各留一份：它是**平台契约的
// 适配器**（把 user-service 的 RPC 翻译成 authz.Resolver），把它抽成共享包会让 platform
// 依赖 contracts 里某个具体的服务版本，而那正是 platform 不该知道的事。
//
// 为什么支付域也要实时取一次授权：本切片只发出 payment:read 一枚码，看着不像「一小时的
// 延迟能白送钱」的会员调整那么急；但这条链是**每个服务都接**的——一个服务为了省一次 RPC
// 去读令牌里的旧 claims，下一个服务照着抄的时候就不会再想一遍自己那枚码管的是什么。
// 统一比每个服务各自判断「我这枚码够不够敏感」更不容易出错。
//
