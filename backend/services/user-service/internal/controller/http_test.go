...
func (f fakeRepository) Create(context.Context, string) (model.User, error) {
	if f.createCalls != nil {
		*f.createCalls++
	}
	return f.user, f.err
}
func (f fakeRepository) GetByID(context.Context, int64) (model.User, error) { return f.user, f.err }
func (f fakeRepository) Update(context.Context, int64, model.UserUpdate) (model.User, error) {
	return f.user, f.err
}

func TestRegister(t *testing.T) {
	h := New(service.NewService(fakeRepository{user: model.User{ID: 1, Name: "Alice", CreatedAt: time.Now()}}))
...
func TestRegisterRejectsTrailingJSONData(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := New(service.NewService(fakeRepository{user: model.User{ID: 1, Name: "Alice"}}))
			res := httptest.NewRecorder()
...
func TestRegisterRejectsOversizedBody(t *testing.T) {
	createCalls := 0
	h := New(service.NewService(fakeRepository{createCalls: &createCalls}))
...
func TestGetByID(t *testing.T) {
	tests := []struct {
		...
		{"not found", "/users/7", model.ErrNotFound, http.StatusNotFound},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := New(service.NewService(fakeRepository{user: model.User{ID: 7, Name: "Alice"}, err: tt.err}))
			res := httptest.NewRecorder()
...
func TestHandlersRejectWrongMethod(t *testing.T) {
	h := New(service.NewService(fakeRepository{}))
	res := httptest.NewRecorder()
	...
	if errors.Is(nil, model.ErrNotFound) {
...
func TestProfileRequiresMatchingBearerUserToken(t *testing.T) {
	...
	h := New(service.NewService(fakeRepository{user: user}))
...
func TestUpdateRejectsInvalidPatchBodies(t *testing.T) {
	for _, body := range []string{"null", `{}`, `{"status":"disabled"}`} {
		t.Run(body, func(t *testing.T) {
			h := New(service.NewService(fakeRepository{user: model.User{ID: 1}}))
			res := httptest.NewRecorder()
