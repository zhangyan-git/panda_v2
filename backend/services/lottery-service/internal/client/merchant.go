...
//
// 这一条与名字解析（Names）的处理**故意不一样**：这里问不到必须让开通失败，因为开通会
// 在抽奖库里永久留下一条记录，而「这家店到底存不存在」我们当时并没有问出来。
func (c *StoreClient) Exists(ctx context.Context, storeID string) (bool, error) {
	...
	if resp.GetStore() == nil {
		// 应答里没有门店又不算 NotFound：这不可能是一次正常回答，当成问不到，别让一个空
		// 应答被读成「存在」——那正好是这条校验唯一不该犯的错。
...
// Names 一次性解出一页门店的名字。
//
// **问不到的名字返回空串而不是错误**：这个返回值只用来显示。商户服务抖一下就让后台的
// 开通列表整页打不开——连「停用一家店的抽奖」都做不了——代价比几个空格子大得多；而空格子
// 也不构成谎言，门店的身份是那一列 id，它来自本服务的库，不来自商户域。
//
