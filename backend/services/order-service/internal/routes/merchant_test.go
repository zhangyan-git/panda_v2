...
const merchantOrderID = "2f8a1f1e-0f4a-4a5e-9c3b-1d2e3f4a5b6c"

// fakeOrderRepo 只实现商户那条路会走到的两个方法，其余留在「没实现」的状态。
//
// 它记下交给查询的过滤器：商户域的订单列表没有门店字段，所以 seenFilter.StoreIDs 是
// 范围唯一能留下的痕迹。
type fakeOrderRepo struct {
	service.Repository
	rows   []*repository.OrderRow
	detail *repository.OrderDetail
	err    error

	seenFilter repository.OrderFilter
	listCalls  int
}

func (f *fakeOrderRepo) ListOrders(_ context.Context, filter repository.OrderFilter) ([]*repository.OrderRow, int, error) {
	f.listCalls++
	f.seenFilter = filter
	if f.err != nil {
		return nil, 0, f.err
...
func (f *fakeOrderRepo) GetOrderDetail(_ context.Context, _ string) (*repository.OrderDetail, error) {
	if f.err != nil {
		return nil, f.err
	}
	if f.detail == nil {
		// 与真仓储同一条出口：查不到订单就该是 404。
...
func newMerchantAPI(t *testing.T, repo *fakeOrderRepo, grants authz.MerchantGrants) *merchantAPI {
	t.Helper()
	...
	// 设备、支付、套餐、钱包四个读端传 nil：这条只读路线一个都不碰，传真的进来只会
	// 把这个用例变成一次集成测试。
	orderService := service.New(repo, nil, nil, nil, nil, nil, service.Options{})
	server := khttp.NewServer()
	RegisterMerchant(runtime.NewHTTPRouter(server),
		controller.NewMerchantOrderController(service.NewMerchantOrderService(orderService)), authorize)
	api.server = server
	return api
}
...
func TestMerchantOrderListIsBoundToTheResolvedScope(t *testing.T) {
	store := "s1"
	repo := &fakeOrderRepo{rows: []*repository.OrderRow{{Order: &model.Order{ID: merchantOrderID, StoreID: &store}}}}
	...
	w := api.call(t, merchantGrant(), http.MethodGet, "/v1/merchant/orders")
...
// 加宽的边界等于没有边界。
func TestMerchantOrderListIgnoresStoreIDsFromTheQuery(t *testing.T) {
	repo := &fakeOrderRepo{}
	api := newMerchantAPI(t, repo, authz.MerchantGrants{
		MerchantID: "m1", ScopeType: auth.ScopeTypeStore, ScopeID: "s1", StoreIDs: []string{"s1"},
	})

	// 带的那个点位刻意不是 UUID：后台那条路径上 ?storeIds= 会被当成 uuid 列去查，
	// 商户这条路径根本不该认这个参数，连校验都不该有。
	w := api.call(t, merchantGrant(), http.MethodGet,
		"/v1/merchant/orders?storeIds=s9&status=paid&orderNo=SC-1")
	if w.Code != http.StatusOK {
		t.Fatalf("status=%d body=%s", w.Code, w.Body)
	}
	...
	if repo.seenFilter.Status != "paid" || repo.seenFilter.OrderNo != "SC-1" {
		t.Fatalf("过滤=%+v，状态与订单号应当照常生效", repo.seenFilter)
	}
}

// 一个点位都没授权的账号是正常账号，不是错误。它产生的过滤器必须是一个空集合，而不是
// 「不过滤」——后者会让它看见全平台的订单。
func TestMerchantOrderListWithEmptyScopeFiltersEverything(t *testing.T) {
	repo := &fakeOrderRepo{}
	api := newMerchantAPI(t, repo, authz.MerchantGrants{
		MerchantID: "m1", ScopeType: auth.ScopeTypeBrand, ScopeID: "b-empty",
	})

	if w := api.call(t, merchantGrant(), http.MethodGet, "/v1/merchant/orders"); w.Code != http.StatusOK {
		t.Fatalf("status=%d body=%s", w.Code, w.Body)
	}
...
func TestMerchantOrderDetailHidesOutOfScopeOrders(t *testing.T) {
	...
	for _, tt := range []struct {
	...
	}{
		{
			name:   "范围之内",
			scope:  []string{"s1", "s2"},
			detail: &repository.OrderDetail{Order: &model.Order{ID: merchantOrderID, StoreID: &inside}},
			want:   http.StatusOK,
		},
		{
			...
			name:   "别的商户的订单",
			scope:  []string{"s1", "s2"},
			detail: &repository.OrderDetail{Order: &model.Order{ID: merchantOrderID, StoreID: &outside}},
			want:   http.StatusNotFound,
		},
		{
			// 没挂点位的订单不在任何账号能授权的集合里。
			name:   "没有点位归属的订单",
			scope:  []string{"s1"},
			detail: &repository.OrderDetail{Order: &model.Order{ID: merchantOrderID}},
			want:   http.StatusNotFound,
		},
		{
			name:    "订单不存在",
			scope:   []string{"s1"},
			repoErr: repository.ErrOrderNotFound,
			want:    http.StatusNotFound,
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			repo := &fakeOrderRepo{detail: tt.detail, err: tt.repoErr}
			api := newMerchantAPI(t, repo, authz.MerchantGrants{
				MerchantID: "m1", ScopeType: auth.ScopeTypeBrand, ScopeID: "b1", StoreIDs: tt.scope,
			})
			w := api.call(t, merchantGrant(), http.MethodGet, "/v1/merchant/orders/"+merchantOrderID)
			if w.Code != tt.want {
...
// 商户域这一轮只有读。写方法不能掉进一个不看方法的处理器里——订单域尤其要紧：这里
// 取消与标记完成都已经有实现，多挂一条路由就是给了商户端一次替用户资产做决定的能力。
func TestMerchantOrderRoutesAreReadOnly(t *testing.T) {
	repo := &fakeOrderRepo{}
	api := newMerchantAPI(t, repo, authz.MerchantGrants{
	...
	paths := []string{
		"/v1/merchant/orders",
		"/v1/merchant/orders/" + merchantOrderID,
		"/v1/merchant/orders/" + merchantOrderID + "/cancel",
		"/v1/merchant/orders/" + merchantOrderID + "/complete",
	}
...
func TestMerchantRoutesRejectNonMerchantCallers(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.noToken {
				w = api.do(t, "", http.MethodGet, "/v1/merchant/orders")
			} else {
				w = api.call(t, tt.grant, http.MethodGet, "/v1/merchant/orders")
			}
			if w.Code != tt.want {
				t.Fatalf("status=%d want %d body=%s", w.Code, tt.want, w.Body)
			}
...
func TestMerchantRoutesFailClosedWhenTheScopeCannotBeResolved(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			repo := &fakeOrderRepo{rows: []*repository.OrderRow{{Order: &model.Order{ID: merchantOrderID}}}}
...
func TestMerchantScopeIsResolvedFreshEveryRequest(t *testing.T) {
	...
	if w := api.do(t, token, http.MethodGet, "/v1/merchant/orders"); w.Code != http.StatusOK {
		t.Fatalf("status=%d body=%s", w.Code, w.Body)
	}
	if !reflect.DeepEqual(repo.seenFilter.StoreIDs, []string{"s1", "s2"}) {
...
func TestMerchantRoutesMapPathsToHandlers(t *testing.T) {
	...
	server := khttp.NewServer()
	RegisterMerchant(runtime.NewHTTPRouter(server), controller.NewMerchantOrderController(nil), probe)

	for _, path := range []string{"/v1/merchant/orders", "/v1/merchant/orders/" + merchantOrderID} {
	...
	// 没挂上来的路径仍然是 404。售后就是其中一条：商户端这一轮不做售后，这条断言把它钉住。
	miss := httptest.NewRecorder()
	server.ServeHTTP(miss, httptest.NewRequest(http.MethodGet, "/v1/merchant/after-sales", nil))
...
func TestMerchantRoutesFailClosedWithoutAnAuthorizer(t *testing.T) {
	server := khttp.NewServer()
	RegisterMerchant(runtime.NewHTTPRouter(server), controller.NewMerchantOrderController(nil), nil)

	r := httptest.NewRequest(http.MethodGet, "/v1/merchant/orders", nil)
	w := httptest.NewRecorder()
	server.ServeHTTP(w, r)
