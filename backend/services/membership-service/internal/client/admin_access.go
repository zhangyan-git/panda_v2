// Package client 是会员服务向外的同步调用。
//
// 这一刀只有一个：问 user-service「这个后台账号现在能做什么」。它是**鉴权链的一环**，
// 不是业务依赖——少了它，后台每一条路由都会 fail-closed 成 503（见 authz 中间件）。
package client
...
// 问题，每个请求问一次。
//
// 与 lottery-service / order-service / coffee-machine-service 里那几个同名类型是同一份实现，
// 各服务各留一份：它是**平台契约的适配器**（把 user-service 的 RPC 翻译成 authz.Resolver），
// 把它抽成共享包会让 platform 依赖 contracts 里某个具体的服务版本，而那正是 platform
// 不该知道的事。
//
// 为什么不读令牌里的 claims：那是登录时签发的，在过期前一直是旧值（access token 是 24
// 小时）。一次被撤销的授权、一个被降级的管理员，会在剩下的整个有效期内继续生效——而
// membership:adjust 这一枚管的正是「谁能把一个人的会员有效期改到明年」，一小时的延迟
// 都不能接受：那是直接白送钱。
//
// admin access RPC 验的是调用者自己的 access token，所以这个客户端**不带服务令牌**：
// 服务令牌是给「代表基础设施去操作」的 RPC 用的。
