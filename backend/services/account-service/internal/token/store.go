...
type Store interface {
	...
	Rotate(context.Context, string, TokenRecord, time.Time) (bool, error)
...
func (s *MemoryStore) Rotate(_ context.Context, oldJTI string, record TokenRecord, now time.Time) (bool, error) {
	...
	defer s.mu.Unlock()
	s.removeExpiredLocked(now)
	old, ok := s.items[oldJTI]
	if !ok || !old.ExpiresAt.After(now) {
		return false, nil
	}
	delete(s.items, oldJTI)
	s.items[record.JTI] = record
	return true, nil
}
