...
type recordingWriter struct{ accounts []domain.Account }

func (w *recordingWriter) Upsert(_ context.Context, account domain.Account) error {
...
func TestInitializeDevelopmentAccountsHashesConfiguredAccounts(t *testing.T) {
	...
	for _, account := range writer.accounts {
		if account.PasswordHash == "admin-secret" || account.PasswordHash == "merchant-secret" || !domain.CheckPassword(account.PasswordHash, map[string]string{"admin": "admin-secret", "merchant": "merchant-secret"}[account.Username]) {
