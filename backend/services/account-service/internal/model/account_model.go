...
// CanonicalRoles returns the stable authorization roles for an account. The
// account type always contributes its canonical role; configured roles are
// retained after trimming and de-duplication.
func CanonicalRoles(account Account) []string {
	canonical := string(account.Type)
	roles := make([]string, 0, len(account.Roles)+1)
	if canonical != "" {
