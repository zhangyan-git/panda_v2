...
--   Settlement 拥有分账与结算（即便同仓部署，库与契约也各自独立，本库不建结算表）
--
-- 两条被别的服务固定、必须对齐的约定：
--
...
--
-- provider 是适配器键（wechat / unionpay / fengxuan / youlian / manual），**故意不加 CHECK**：
-- 接一个新渠道应该是插一行配置，不是开一次迁移改约束——这与 status 那种封闭状态机不同。
--
